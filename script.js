...
function applyCardBackColors() {
  ...
  allCards.forEach(card => {
    ...
    const back = card.querySelector(".album-back");
    if (!img || !back) return;
    back.style.transition = "background 0.5s ease";

    const fallback = "rgba(255, 211, 107, 0.4)";

    const setColor = () => {
      try {
        const [r, g, b] = colorThief.getColor(img);
        back.style.background = `rgba(${r}, ${g}, ${b}, 0.6)`;
      } catch {
        back.style.background = fallback;
      }
    };

    if (img.complete) setColor();
    else img.onload = setColor;
  });
}
